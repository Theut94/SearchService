...
namespace WordSearcher
{
    public class Database
    {
        ...
        private async Task ExecuteAsync(IDbConnection connection, string sql)
        {
            ...
            {
                Console.WriteLine($"We failed the command : {sql} on connection {connection}");
            }
        }
        ...
        public async Task<Dictionary<string, int>> GetAllWords()
        {
            ...
            var connection = coordinator.GetOccurrenceConnection();
            connection.Open();
            SqlCommand selectCmd = (SqlCommand)connection.CreateCommand();
        ...
        public async Task<List<string>> GetDocDetails(List<int> docIds)
        {
            ...
            var selectCmd = connection.CreateCommand(); selectCmd.CommandText = "SELECT * FROM Documents WHERE id IN " + AsString(docIds);
